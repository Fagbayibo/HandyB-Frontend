...
export default function Login() {
  ...
  const validateField = async (field, value) => {
    try {
      await loginSchema.validateAt(field, { phone, password, [field]: value });
      ...
      <div className="w-full lg:w-1/2 overflow-y-auto p-6 sm:p-10 lg:p-16 flex flex-col items-center mt-6 sm:mt-12 space-y-6">
        ...
        <div className="flex items-center space-x-4 sm:space-x-6 lg:space-x-10 mt-5 px-4 sm:px-6 bg-lightgray py-2 rounded-full">
          ...
          <div className="flex items-center justify-center space-x-3 sm:space-x-4">
            {[GoogleIcon, Apple, FacebookIcon].map((icon, i) => (
              <div
                key={i}
                className="w-10 h-10 sm:w-12 sm:h-12 bg-white rounded-full flex items-center justify-center cursor-pointer"
              >
                <img src={icon} alt="icon" className="w-6 sm:w-8" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full max-w-[480px] mt-4">
          ...
          <div className="w-full text-center my-6 sm:my-8">
            <span className="text-sm sm:text-md tracking-tight">
